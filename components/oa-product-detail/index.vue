  ...
  <view class="oa-product-detail">
		...
		<view class="detail" v-if="product.name">
			...
			<view class="introduce-section">
				...
				<view class="product-tag">
					...
					<uni-tag
						...
						circle
						type="base"
						:text="`${product.marketing && product.marketing.min_num}起购`"
						size="small"
					/>
					<uni-tag
						v-if="product.marketing && product.marketing.max_num && type === 'group_buy'"
						...
						circle
						type="base"
						:text="`限购${product.marketing && product.marketing.max_num}`"
						size="small"
					/>
					<uni-tag
						v-if="type === 'wholesale'"
				...
				<view class="oa-time-down">
					<oa-count-down
						v-if="type === 'group_buy' || type === 'bargain' || type === 'discount'"
						:show-day="true"
						tip="距结束"
						:second="second(product.marketing && product.marketing.end_time)"
						color="#FFFFFF"
			...
			<!--<view class="share-section" v-if="product.name">-->
				<!--<view class="share-icon">-->
					...
					<!--返-->
				<!--</view>-->
				<!--<text class="tit">分享该商品给您的朋友们</text>-->
			...
			<block v-if="type === 'wholesale'">
				<!--拼团公告-->
				<oa-swiper-slide @navTo="navTo(`/pages/marketing/wholesale/group?id=${product.id}&state=2`)" class="oa-swiper-slide" :list="wholesaleSuccess && wholesaleSuccess.list" v-if="wholesaleSuccess && wholesaleSuccess.list.length > 0">
					<view slot="header">
			...
			<view class="c-list">
				...
				<!--限购说明-->
				<oa-item-popup title="限购说明" v-if="type === 'buy_now' && parseInt(product.max_buy, 10) > 0">
					<view slot="content">
						<text>{{ `${product.max_buy} ${product.unit || '件'}` }}</text>
					</view>
				</oa-item-popup>
				<!--积分活动-->
				<oa-item-popup title="积分活动" v-if="product.point_exchange_type !== '1'">
					<view slot="content" class="con-list">
			...
			<view class="detail-desc">
				...
				<oa-parser :html="product.intro" lazy-load></oa-parser>
			</view>
			<!-- 底部操作菜单 -->
			<view class="page-bottom">
				<view class="page-bottom-bth-wrapper">
					...
					<view @tap="navTo(`/pages/service/index`)" class="p-b-btn">
		...
		>
			...
			<view class="layer" @tap.stop="stopPrevent">
				...
				<scroll-view class="oa-list" scroll-y>
					<view
						class="oa-list-item"
					...
					<view class="no-data" v-if="addressList.length === 0">
						{{ tabCurrentIndex === 1 ? '请进入个人中心-我的服务-收货地址添加地址' : '请联系管理员添加自提点' }}
					</view>
				</scroll-view>
			</view>
		</view>
		...
		<!--回到顶部-->
		<oa-live v-if="product.name"></oa-live>
		<!--#ifdef MP-->
    <oa-nav></oa-nav>
		<!--#endif-->
		<view
		...
		<!--海报分享-->
		<oa-poster :show="posterShow" :poster="poster.itempFilePathmg" @hidePoster="hidePoster"></oa-poster>
  </view>
</template>
<script>
	/**
	 ...
	 */
	import rfItemPopup from '@/components/oa-item-popup';
	import moment from '@/common/moment';
	import rfAttrContent from '@/components/oa-attr-content';
	import rfPoster from '@/components/oa-poster';
	import { getSharePoster } from '@/utils/poster/QS-SharePoster';
	import rfRate from '@/components/oa-rate/oa-rate';
	import rfBadge from '@/components/oa-badge/oa-badge';
	import uniTag from '@/components/uni-tag/uni-tag';
	import rfNav from '@/components/oa-nav';
	import rfLive from '@/components/oa-live';
  ...
  import { couponReceive, addressList } from '@/api/userInfo';
	import rfCountDown from '@/components/oa-count-down';
    ...
    methods: {
      ...
      share() {
        ...
        // #ifdef APP-PLUS
			...
			// 通用跳转
			navTo(route) {
				if (this.appServiceType === '1' && route === '/pages/service/index') {
					this.kefuShow = true;
					return;
				}
				if (!this.hasLogin) {
