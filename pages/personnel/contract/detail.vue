<template>
	<view class="page">
		...
		<view class="input-t ">
			<text>名称</text>
			<input class="input tit" type="text" v-model="sendDate.title" placeholder="请输入合同名称,不要超过25个字符" placeholder-class="placeholder" />
		</view>
		<view class="input-t ">
			<text>甲方</text>
	...
	export default {
		...
		methods: {
			...
			async send() {
				...
				await this.$http
					.put(`${contract}/update?id=${this.id}`, {
						...this.sendDate
					})
					.then(() => {
