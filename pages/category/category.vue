<template>
	<view class="oa-category">
		...
		<view v-if="styleCateType === 'one_cover'" class="one-cover-category">
			<view
				class="box"
				v-for="tItem in categoryList"
				:key="tItem.id"
		...
		<view
			...
			v-if="(styleCateType === 'one_two_three_cover' || styleCateType === 'one_two_three_text' || styleCateType === 'one_two_text' || styleCateType === 'one_two_cover') && categoryList.length > 0">
			<!-- 左侧分类导航 -->
			...
			<scroll-view scroll-y="true" class="right">
				...
				>
					<view
						...
						<block v-if="styleCateType === 'one_two_three_cover' || styleCateType === 'one_two_three_text'">
							<view
								class="box"
						...
						<block v-if="styleCateType === 'one_two_cover' || styleCateType === 'one_two_text'">
							<view class="list">
								<view
									class="box"
									...
									:key="sItem.id"
								>
									<block v-if="styleCateType === 'one_two_cover'">
										<image :src="sItem.cover || errorImage"></image>
										<view class="text">{{ sItem.title }}</view>
									</block>
									<text class="category-tag in1line" v-if="styleCateType === 'one_two_text'">
										{{ sItem.title }}
									</text>
								</view>
							</view>
		...
		<view class="category-list" v-if="(styleCateType === 'one_product' || styleCateType === 'one_two_product') && categoryList.length > 0">
			<!-- 左侧分类导航 -->
			<scroll-view scroll-y="true" class="left">
				<view
					class="row"
			...
			<!--右侧子导航-->
			<scroll-view scroll-y="true" class="right" @scrolltolower="getMoreProductList">
				<scroll-view :scroll-x="true" class="second-category" v-if="styleCateType === 'one_two_product'">
					<text
						class="second-category-tag"
						v-for="(sItem, sIndex) in currentSecondCategoryList"
						@click="handleCategoryInvertedChange(sItem, sIndex)"
						:class="categoryInvertedIndex !== sIndex ? 'second-category-normal' : 'bg-' + themeColor.name"
						:key="sIndex"
					>{{ sItem.title }}</text>
				</scroll-view>
				<view
				...
				>
					<view
						...
						<!--<view-->
							<!--class="banner"-->
							<!--@tap="indexTopToDetailPage(cateTop.jump_type, cateTop.jump_link)"-->
						<!--&gt;-->
							<!--<image :src="cateTop && cateTop.cover" mode="aspectFill" />-->
						<!--</view>-->
						<view class="order-by-wrapper">
							<text>推荐</text>
							<view class="order-by">
								<view class="order-by-item" @tap.stop="handleSalesOrderByChange">
									<text>销量</text>
									<view class="iconfont-group">
										<text class="iconfont iconshangsanjiao" :class="total_sales === 'asc' ? 'text-'+themeColor.name : ''"></text>
										<text class="iconfont iconxiasanjiao" :class="total_sales === 'desc' ? 'text-'+themeColor.name : ''"></text>
									</view>
								</view>
								<view class="order-by-item" @tap.stop="handlePriceOrderByChange">
									<text>价格</text>
									<view class="iconfont-group">
										<text class="iconfont iconshangsanjiao" :class="price === 'asc' ? 'text-'+themeColor.name : ''"></text>
										<text class="iconfont iconxiasanjiao" :class="price === 'desc' ? 'text-'+themeColor.name : ''"></text>
									</view>
									<!--<text>价格</text> <text v-if="price" class="iconfont" :class="price === 'asc' ? 'iconxia' : 'iconshang'"></text>-->
								</view>
							</view>
						</view>
						<view>
							<view v-for="(item,index) in categoryProductList" :key="index" class="category-product-list">
								<!--商品列表-->
								<view class="tui-pro-item tui-flex-list" @tap.stop="navTo(`/pages/product/product?id=${item.id}`)" hover-class="hover" :hover-start-time="150">
									<image :src="item.picture" class="tui-pro-img tui-proimg-list" mode="widthFix" />
									<view class="tui-pro-content">
										...
										<view class="tui-pro-pay">
											<view class="tui-pro-price">
												<text class="tui-sale-price" :class="'text-'+themeColor.name">{{ moneySymbol }}{{item.price}}</text>
												<text class="tui-factory-price" v-if="item.market_price > item.price">{{ moneySymbol }}{{item.market_price}}</text>
											</view>
							...
							></oa-load-more>
							<view v-if="categoryProductList.length === 0 && !productLoading" class="no-data">
								<i class="iconfont icon404"></i>
								暂无商品
							</view>
						</view>
...
import rfAttrContent from '@/components/oa-attr-content';
