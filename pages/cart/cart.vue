	...
	<view class="cart">
		...
		<view class="goods-list" v-else>
			<view class="btn-clear" :class="'text-'+themeColor.name" @tap="clearCart({ lose_status: 1 })"
				>清空失效商品</view
			>
			<view class="oa-cart-row" v-for="(row, index) in cartList" :key="index">
				<!-- 删除按钮 -->
								...
								<view class="number" v-if="parseInt(row.status, 10) === 1">
									...
									<view class="input" @tap.stop="discard">
										<input
											...
											v-model="row.number"
											min="1"
											@blur="numberBlur(row, $event, index)"
		...
		>
			...
			<view class="settlement">
				<view class="sum in1line">合计:<text class="money">{{ moneySymbol }}{{ sumPrice | priceFilter }}</text></view>
...
export default {
  components: { rfAttrContent },
	...
	methods: {
		...
		// 控制可输入购物车商品数量
		discard() {},
		async numberBlur(item, data, index) {
