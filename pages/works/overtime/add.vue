<template>
	<view class="page">
		<view class="input-t" @tap.stop="chooseType">
			<text>加班类型</text>
			<text class="input-quick">{{ mtype || '请选择加班类型' }} <text class="iconfont iconxia"></text></text>
		</view>
		<view class="input-t ">
			<text>加班天数</text>
		...
		<view class="input-body">
			<textarea placeholder="请详细描述您要加班的内容或理由..." v-model="sendDate.content" class="input-textare" />
			</view>
		<view class="input-t">
			<text>提醒方式</text>
		</view>
...
<script>
import { overtime,overtimeType } from '@/api/basic';
...
// 监听日期更改
		...
		async send() {
			...
			this.sendDate.status = 'overtime/apply';
