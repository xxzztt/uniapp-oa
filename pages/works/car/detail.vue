	...
	<view class="page">
		...
		<view class="input-title ">
			...
			<template v-for="(cates, index) in workType">
				<template v-if="cates.key==getDetail.type">{{ cates.value }}</template>
			</template>
		</view>
		<view class="input-title ">
			<text >用车时间</text>
			{{getDetail.begin_date }}至{{getDetail.end_date }}
		</view>
		<view class="input-title">
			<text>用车理由</text>
		</view>
...
<script>
import { carApply ,workState,getCarType} from '@/api/basic';
