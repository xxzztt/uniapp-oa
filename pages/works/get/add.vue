<template>
	<view class="page">
		<view class="input-t">
			<text>申领资产</text>
			<text class="input-quick">{{ asset.title || '请选择要申领的资产' }} </text>
		</view>
		<view class="input-t ">
			<text>数量</text>
		...
		<view class="input-body">
			<textarea placeholder="请详细描述您要申领的原因..." v-model="sendDate.content" class="input-textare" />
			</view>
		<view class="input-t">
			<text>提醒方式</text>
		</view>
...
<script>
import { get,asset } from '@/api/basic';
export default {
	data() {
		return {
			type:'',
	...
	methods: {
		...
		async send() {
			...
			this.sendDate.status = 'get/apply';
			await this.$http
				.post(`${get}`+`/create`, {
					...this.sendDate
				})
